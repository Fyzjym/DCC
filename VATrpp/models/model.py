...
class VATr(nn.Module):
    ...
    def save_images_for_fid_calculation_V(self, path, loader, split='train', allImg=False, realImg=False):
        ...
        for step, data in (enumerate(loader)):
            ...
            self.eval_text_encodeF, self.eval_len_textF, _ecoder_p = self.netconverter.encode(texts)
            # self.eval_text_encodeF = self.eval_text_encodeF.to(self.args.device).repeat(batch_size, 1, 1)
            self.eval_text_encodeF = self.eval_text_encodeF.to(self.args.device).unsqueeze(1)
            # self.eval_textImg_encodeF = self.textImgList['timg'].repeat(batch_size, 1, 1, 1).to(self.args.device)

            # self.fakes = self.netG.Eval(ST, self.eval_text_encodeF, self.eval_textImg_encodeF)
            # self.fakes = self.netG.Eval(ST, self.eval_text_encodeF)
            self.fakes = self.netG.evaluate(ST, self.eval_text_encodeF)
            # self.fakes = self.netG.Eval(ST, self.eval_text_encode)
            # print(self.fakes[0].shape)
            fake_images = torch.cat(self.fakes, 1).detach().cpu().numpy()
            # fake_images = self.fakes[1].detach().cpu().numpy()
            # printShape(self.fakes )
