...
def generate_fid(args):
    ...
    if not args.all_epochs:
        c_epoch = args.checkpoint.split('/')[-1].split('_')[0]
        writer = Writer(args.checkpoint, args, only_generator=True)
        if not args.test_only:
            writer.generate_fid(args.output, train_loader,
                                model_tag=model_tag, split='train',
                                fake_only=args.fake_only, long_tail_only=args.long_tail,
                                all_img_flag = args.all_img,
                                epoch_n = c_epoch)

        writer.generate_fid(args.output, test_loader,
                            model_tag=model_tag, split='test',
                            fake_only=args.fake_only, long_tail_only=args.long_tail,
                            all_img_flag = args.all_img,
                            epoch_n = c_epoch)
